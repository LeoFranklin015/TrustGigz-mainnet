...
const Navbar = () => {
  ...
  const handleFetchWeb3Name = useCallback(
    async (address: string) => {
      // Prevent multiple fetches for the same address
      if (fetchedAddress === address) return;
      ...
      try {
        ...
        // Reset if fetch fails
        setFetchedAddress(null);
      }
    },
    [web3name, fetchedAddress]
  );
        ...
        <div className="flex items-center space-x-4">
          <ConnectButton.Custom>
            {({
              ...
              // Only fetch Web3 name when account is truly connected
              React.useEffect(() => {
                if (
                  mounted &&
                  account &&
                  ...
                  handleFetchWeb3Name(account.address);
                }
              }, [account?.address, handleFetchWeb3Name, mounted]);
